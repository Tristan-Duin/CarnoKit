...
class UpdaterCog(commands.GroupCog, group_name="update"):
    ...
    @app_commands.command(name="check", description="Check if a server update is needed")
    @require_admin
    ...
    @app_commands.command(name="status", description="Show game build update info")
    ...
    @app_commands.command(name="now", description="Force an immediate server update check/restart with countdown")
    @require_owner
    async def now(self, interaction: discord.Interaction):
        view = ConfirmUpdateView()
        await interaction.response.send_message(
            embed=embeds.warning(
                "Force Cluster Server Refresh",
                (
                    f"This will start a **{cfg.update_countdown_minutes}-minute** countdown, "
                    f"then save and restart **every map** in the cluster.\n\n"
                    f"Continue?"
                ),
            ),
            view=view,
